...
def grayscale(img_matrix):
    for i in range(len(img_matrix)):
        for j in range(len(img_matrix[i])):
            sum_pixel = int((img_matrix[i][j][0]+img_matrix[i][j][1]+img_matrix[i][j][2])/3)
            for k in range(len(img_matrix[i][j])):
                img_matrix[i][j][k] =  sum_pixel
    return img_matrix
...
def rotate(img_matrix):
    new_list=copy.deepcopy(img_matrix)
    for x in range(len(img_matrix)):
        for y in range(len(img_matrix[x])):
            new_list[x][y] = img_matrix[len(img_matrix)- x -1][len(img_matrix[0])-y-1]
    return new_list
...
def edge_detect(img_matrix):
    ...
    for x in range(len(img_matrix)):
        for y in range(len(img_matrix[x])):
            for k in range(len(img_matrix[x][y])):
                new_list[x][y][k] = 0
                
    for x in range(1,len(img_matrix) - 1):
        for y in range(1,len(img_matrix[x]) - 1):
            for k in range(len(img_matrix[x][y])):
                a =  8* img_matrix[x][y][k]
                b = img_matrix[x][y-1][k] + img_matrix[x][y+1][k] + img_matrix[x-1][y][k] + img_matrix[x-1][y+1][k]   
                b1 = img_matrix[x-1][y-1][k] + img_matrix[x+1][y][k] + img_matrix[x+1][y-1][k] + img_matrix[x+1][y+1][k]
                new_list[x][y][k] =  a - b - b1
                if new_list[x][y][k] < 0:
                    new_list[x][y][k] = 0
                if new_list[x][y][k] > 255:
                    new_list[x][y][k] = 255
    return new_list
