...
def hw8_index(row1_str):
    try:
        new_string=row1_str.split(',')
        return new_string.index('hw8 Grade')
    except ValueError :
        return -1
        #print(row1_str.index('hw8 Grade'))
...
def alter_grade(row_str,idx):
    new_string=row_str.split(',')
    new_string[idx]='40'
    new_string_2= ','.join(new_string)
    return new_string_2
...
def haxx(fname):
    data=get_data_list(fname)
    if data==-1:
        return False
    index=hw8_index(data[0])
    if index==-1:
        return False
    fp=open(fname,'w')
    for i in data:
